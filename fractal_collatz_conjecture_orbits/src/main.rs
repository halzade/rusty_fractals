...
use rusty_fractals_common::fractal::{Conf, FractalCommon, FractalConfig, FractalMath, FractalNebulaCommon};
...
impl FractalNebulaCommon for CollatzConjectureOrbits<'_> {
...
impl FractalCommon for CollatzConjectureOrbits<'_> {
    fn name(&self) -> &'static str { "CollatzConjectureOrbits" }
...
pub static FRACTAL: Mutex<Option<CollatzConjectureOrbits>> = Mutex::new(None);
...
fn main() {
    ...
    let mut fractal: CollatzConjectureOrbits<'static> = CollatzConjectureOrbits { app: application };
...
mod tests {
    ...
    #[test]
    fn test_math() {
        let collatz = CollatzConjectureOrbits { app: application::init_none() };
